...
const AccountPoolSwitcher = ({
  ...
  const tierMetaMap = useMemo(() => {
    return new Map(buildManualFailoverTierSummary(sortedAccounts).map(item => [item.priority, item]));
  }, [sortedAccounts]);
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return sortedAccounts.filter((account) => {
      ...
      return accountName.includes(keyword) || authLabel.includes(keyword) || planLabel.includes(keyword);
    });
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        title="切换即设为主组，影响后续 /v1/responses 调度"
      >
        <span className="relative flex h-2 w-2">
          <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${activeIndicatorClass} opacity-75`}></span>
          <span className={`relative inline-flex rounded-full h-2 w-2 ${activeIndicatorClass}`}></span>
        </span>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-[360px] bg-white rounded-xl shadow-xl border border-gray-100 ring-1 ring-black/5 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="px-4 py-3 bg-gray-50 border-b border-gray-100">
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider">切换 Codex 账号</div>
            <div className="text-[10px] text-gray-400 mt-0.5">切换即设为主组，影响后续 /v1/responses 调度</div>
          </div>
          ...
          <div className="p-2 max-h-[360px] overflow-y-auto">
            {filteredAccounts.length === 0 ? (
              ...
              filteredAccounts.map((account) => {
                ...
                const tierMeta = tierMetaMap.get(priority);
                    ...
                    <div className="flex items-start gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex items-center gap-2 flex-wrap text-[10px] text-gray-500">
                          {tierMeta?.label && (
                            <span className="px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-600">{tierMeta.label}</span>
                          )}
                          {Number.isFinite(priority) && <span>P{priority}</span>}
