      ...
      >
        <div className="border-b border-slate-100 bg-gradient-to-r from-slate-50 via-white to-white px-6 py-5">
          <div className="flex items-start justify-between gap-4">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <h2 className="text-lg font-semibold text-slate-900">Codex 模型目录</h2>
                <p className="mt-0.5 text-sm text-slate-500">GET /v1/models 本地目录</p>
              </div>
            </div>
            <button
              type="button"
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <span className={`rounded-full border px-2.5 py-1 text-xs font-medium ${catalog.enabled ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-slate-100 text-slate-500'}`}>
              ...
              <div className="grid gap-4 lg:grid-cols-[260px_1fr]">
                <div className="space-y-3">
                  <label className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
                    <span>
                      <span className="block text-sm font-medium text-slate-700">本地接管 /v1/models</span>
                    ...
                    <button
                      ...
                      disabled={saving}
                      className={`relative h-6 w-11 rounded-full transition-colors disabled:opacity-50 ${catalog.enabled ? 'bg-indigo-500' : 'bg-slate-300'}`}
                    >
                      <span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow-sm transition-all ${catalog.enabled ? 'right-1' : 'left-1'}`} />
                    </button>
                  </label>

                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                    <span className="mb-2 block text-xs font-medium text-slate-500">添加自定义模型</span>
                    <div className="flex gap-2">
                      <input
                        ...
                        className="min-w-0 flex-1 rounded-md border border-slate-200 bg-white px-2 text-sm text-slate-700 focus:border-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-900"
                      />
                      <button
                        type="button"
                        onClick={handleAddModel}
                        disabled={saving}
                        className="flex h-9 w-9 items-center justify-center rounded-md bg-slate-900 text-white transition-colors hover:bg-slate-800 disabled:opacity-50"
                        title="添加模型"
                      >
                        <Plus size={16} />
                      </button>
                ...
                <div className="overflow-hidden rounded-lg border border-slate-200">
                  <div className="grid grid-cols-[minmax(0,1fr)_90px_88px_92px] border-b border-slate-100 bg-slate-50 px-3 py-2 text-xs font-medium text-slate-500">
                    <span>模型</span>
                    <span>来源</span>
                    <span>状态</span>
                  ...
                  {loading ? (
                    ...
                    <div className="px-3 py-8 text-center text-sm text-slate-400">暂无模型</div>
                  ) : (
                    <div className="divide-y divide-slate-100">
                      {modelRows.map(model => (
                        <div key={model.id} className="grid grid-cols-[minmax(0,1fr)_90px_88px_92px] items-center px-3 py-2.5 text-sm">
                          <div className="min-w-0">
                            <div className="truncate font-mono text-[13px] font-medium text-slate-800">{model.id}</div>
                            {(model.display_name && model.display_name !== model.id) || model.deprecated ? (
                              <div className="mt-0.5 flex items-center gap-1.5 text-[11px] text-slate-400">
                                ...
                                {model.deprecated && <span className="rounded bg-amber-50 px-1.5 py-0.5 text-amber-700">deprecated</span>}
                              </div>
                            ) : null}
                          </div>
                          <span className={`w-fit rounded-full px-2 py-0.5 text-[11px] font-medium ${model.source === 'official' ? 'bg-sky-50 text-sky-700' : 'bg-violet-50 text-violet-700'}`}>
                          ...
                          <button
                            ...
                            disabled={saving}
                            className={`h-7 w-fit rounded-full px-2 text-[11px] font-medium transition-colors ${model.enabled ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}
                          >
                            {model.enabled ? '启用' : '禁用'}
                          </button>
                          <div className="flex justify-end">
                            {model.source === 'custom' && (
                              <button
                                ...
                                disabled={saving}
                                className="flex h-8 w-8 items-center justify-center rounded-md text-slate-400 transition-colors hover:bg-rose-50 hover:text-rose-600 disabled:opacity-50"
                                title="删除自定义模型"
                              >
                                <Trash2 size={15} />
                              </button>
            ...
            <div className="flex flex-col gap-3 border-t border-slate-100 bg-white px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button
                  ...
                  className={`${compactButtonClass} border-slate-200 bg-white text-slate-600 hover:bg-slate-50`}
                >
                  <RefreshCw size={14} />
                  刷新
                </button>
                ...
                <button
                  ...
                  disabled={loading || saving}
                  className={`${compactButtonClass} border-rose-100 bg-rose-50 text-rose-700 hover:bg-rose-100`}
                >
                  <RotateCcw size={14} />
                  恢复预设
                </button>
                <button
                  ...
                  className={`${compactButtonClass} border-slate-900 bg-slate-900 text-white hover:bg-slate-800`}
                >
                  <Save size={14} />
                  保存
                </button>
