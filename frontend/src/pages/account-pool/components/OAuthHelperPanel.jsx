...
const OAuthHelperPanel = ({
  ...
  <div className="rounded-lg border border-success-line bg-success-soft/60 p-3 space-y-3">
    <button
      type="button"
    ...
    {oauthSectionExpanded && (
      <div className="space-y-3 border-t border-success-line/80 pt-3">
        ...
        {oauthSession?.auth_url && (
          <div className="space-y-1">
            <div className="text-xs text-fg-body">授权链接</div>
            <textarea
              value={oauthSession.auth_url}
              readOnly
              rows={2}
              className="w-full rounded-lg border border-line bg-surface px-3 py-2 text-xs font-mono"
            />
          </div>
        )}

        <div className="space-y-1">
          <div className="text-xs text-fg-body">回调 URL</div>
          <textarea
            value={oauthCallbackURL}
            onChange={(event) => setOauthCallbackURL(event.target.value)}
            rows={2}
            className="w-full rounded-lg border border-line px-3 py-2 text-xs font-mono focus:border-accent-line focus:outline-none focus:ring-2 focus:ring-accent-soft"
