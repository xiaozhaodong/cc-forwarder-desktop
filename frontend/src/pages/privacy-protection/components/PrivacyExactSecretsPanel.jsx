  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] flex items-start justify-center px-4 pt-[15vh]">
      ...
      <form
        ...
        className="relative flex max-h-[75vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-line bg-surface shadow-2xl"
      >
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col">
              <label className="mb-1.5 text-sm font-medium text-fg-body">分类</label>
              <CustomSelect
          ...
          <div className="flex items-center justify-between rounded-lg border border-line px-3 py-2">
            <div>
              <div className="text-sm font-medium text-fg-body">启用</div>
              <div className="text-xs text-fg-subtle">关闭后保留记录但不参与出站扫描</div>
            </div>
            <Toggle checked={form.enabled} disabled={saving} onChange={(enabled) => update({ enabled })} />
          </div>
          <div className="flex flex-col">
            <label className="mb-1.5 text-sm font-medium text-fg-body">描述</label>
            <textarea
              value={form.description}
              onChange={(e) => update({ description: e.target.value })}
              rows={3}
              className="w-full rounded-lg border border-line px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-ring"
            />
          </div>
          {form.id > 0 && (
        ...
        <div className="border-t border-line-soft bg-surface px-6 py-4">
          {submitError && <p className="mb-2 break-all text-xs text-danger">{submitError}</p>}
...
const ImportCandidatesDialog = ({ open, loading, candidates, onLoad, onImport, onClose }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-overlay pt-[12vh] animate-fade-in">
      <div
        role="dialog"
        aria-modal="true"
        aria-label="导入候选"
        className="mx-4 w-full max-w-2xl overflow-hidden rounded-2xl bg-surface shadow-xl"
      >
        <div className="flex items-center justify-between border-b border-line-soft px-5 py-4">
          ...
          <button
            ...
            aria-label="关闭弹窗"
            onClick={handleRequestClose}
            className="rounded-lg p-1.5 text-fg-subtle hover:bg-surface-mut hover:text-fg-body"
          >
            <X size={18} />
          </button>
        </div>
        <div className="max-h-[60vh] overflow-y-auto px-5 py-4">
          ...
          {error && <p className="mb-3 break-all text-xs text-danger">{error}</p>}
          <div className="space-y-2">
            {candidates.map((candidate) => {
              const key = `${candidate.source_type}:${candidate.source_ref}`;
              return (
                <div key={key} className="flex items-center gap-3 rounded-lg border border-line px-3 py-2">
          ...
          {!loading && candidates.length === 0 && (
            <p className="py-6 text-center text-sm text-fg-subtle">暂无可导入候选</p>
          )}
        </div>
      </div>
    </div>,
...
const ManualImportDialog = ({ open, onImport, onClose }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-overlay pt-[10vh] animate-fade-in">
      <form
        onSubmit={handleSubmit}
        role="dialog"
        aria-modal="true"
        aria-label="粘贴导入"
        className="mx-4 flex max-h-[80vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-surface shadow-xl"
      >
        <div className="flex items-center justify-between border-b border-line-soft px-5 py-4">
          <h3 className="flex items-center gap-2 text-base font-semibold text-fg">
            <Upload size={17} className="text-accent" />
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
          ...
          <div className="flex flex-col">
            <label className="mb-1.5 text-sm font-medium text-fg-body">敏感值列表 *</label>
            <textarea
              value={form.values}
              onChange={(e) => update({ values: e.target.value })}
              rows={9}
              placeholder="每行一个完整敏感值，空行会自动跳过"
              className="w-full rounded-lg border border-line px-3 py-2 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-accent-ring"
            />
            <div className="mt-1 text-xs text-fg-subtle">导入前会逐行 TrimSpace，并按完整字符串精确匹配。</div>
          </div>
          <div className="flex flex-col">
            <label className="mb-1.5 text-sm font-medium text-fg-body">描述</label>
            <textarea
        ...
        <div className="border-t border-line-soft bg-surface-sub px-5 py-4">
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-fg-subtle">完整敏感值不在列表中展示，也不会导出到规则 JSON。</span>
      </div>
      {panelError && <p className="break-all text-sm text-danger">{panelError}</p>}
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-line bg-surface px-3 py-2">
          <div className="relative min-w-[220px] flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-line py-1.5 pl-8 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent-ring"
            />
          </div>
          <CustomSelect
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredSecrets.map((secret) => (
                <tr key={secret.id} className="border-t border-line-soft hover:bg-surface-sub">
                  <td className="px-3 py-2">
                    <Toggle checked={secret.enabled} disabled={actionBusy} onChange={(enabled) => handleToggle(secret, enabled)} />
                  </td>
                  <td className="px-3 py-2">
                    <div className="font-medium text-fg">{secret.name}</div>
                    {secret.description && <div className="line-clamp-1 text-xs text-fg-subtle">{secret.description}</div>}
                  </td>
                  <td className="px-3 py-2 text-fg-body">{exactSecretCategoryLabel(secret.category)}</td>
                  <td className="px-3 py-2 text-xs text-fg-muted">
                    <span className="font-mono">{secret.masked_value}</span>
                    <span className="ml-2 text-fg-subtle">{secret.value_length} 字符</span>
                  </td>
                  <td className="break-all px-3 py-2 text-fg-body">{secret.placeholder}</td>
                  <td className="px-3 py-2 text-xs text-fg-subtle">{secret.source_type}</td>
                  <td className="px-3 py-2">
                    <div className="flex justify-end gap-1">
                      <button
                        ...
                        className="rounded-lg p-1.5 text-fg-subtle hover:bg-accent-soft hover:text-accent"
                        title="编辑"
                      >
                        <Pencil size={14} />
                      </button>
