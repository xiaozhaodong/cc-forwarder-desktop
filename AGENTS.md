...
- **internal/store/account_pool.go**：`upstream_accounts` 存储层；账号画像与 quota 字段变更需同步这里与 schema。
- **config/**：示例与运行配置，`config/example.yaml` 为基础模板，生产部署请复制为 `config/config.yaml`。
- **data/** 与 `docs/`：分别存储运行时数据库、设计文档；保证数据目录可写。
- **frontend/src/pages/account-pool/**：Codex 账号池前端页面，展示授权方式、优先级、账号类型、`5h` / `d7`、状态与最近刷新。
